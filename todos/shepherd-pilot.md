...
`~/.shepherd`. It is not the workstream manager described in
[the manager mode map](../docs/manager-mode-map.html). The remaining slices —
a Shepherd-owned headless loop and any UI — are still open.

## The idea

A **pilot** is an agent Shepherd owns and drives, whose job is to operate
Shepherd itself through the `shepherd` CLI. It reads `shepherd list --json`, organizes
workstreams and sessions, maintains persistent notes, and starts or steers work
on the user's behalf. It is rooted in the Shepherd data directory.
...
The pilot's domain is exactly the domain Shepherd already has truth about.
...
The read surface is already sufficient. `shepherd list --json` returns workstream `id`,
`name`, `description`, `artifact_dir`, `roots`, and `revision`, plus per-session
`state`, `title`, `display_title`, `initial_prompt`, `latest_via_shepherd`,
...
| --- | --- |
| `Start` | `shepherd spawn` |
| `Send` | `shepherd send` |
| `Stop` | `shepherd stop` |
| `AttachCommand` | `shepherd attach` |
...
| `SetSessionTitle` | `shepherd title` |
| `AddRoot` | `shepherd ws root add` |
| `ReplaceRoot` | `shepherd ws root set` |
| `RemoveRoot` | `shepherd ws root rm` |
| `DeleteSession` | `shepherd delete --yes` |
...
makes Shepherd scriptable by humans, not just by agents.

## The pilot is not a tmux session

The obvious design is "the pilot is an ordinary Shepherd session." That is wrong,
...
Shepherd `exec`s directly into the runner rather than going through a shell, so
...
`shepherd.Session` carries `PaneID`, `AttachedClients`, `PaneInMode`,
...
Shepherd's honesty rule is *do not claim semantic state you cannot prove*. Driving
the pilot loop directly satisfies that rule harder than tmux does: Shepherd owns
...
> A headless runner is permitted only for an agent Shepherd itself owns, and such
...
`--output-format stream-json`. Shepherd already allocates a UUID before launch and
already supplies it to Claude as its session ID, so pilot conversation
continuity across dashboard restarts uses identity Shepherd owns. Codex headless
support needs the same verification before it is offered.

## Blast radius, stated plainly

The pilot is rooted in the Shepherd data directory, so it cannot edit project code
directly. But `shepherd spawn` starts native agents in project roots registered on
...
surfaces as a Shepherd-rendered confirmation the human presses, not a sentence the
agent writes — which is only possible because Shepherd owns the loop.

Be honest about the limit: owning the loop is necessary, not sufficient. A
headless agent with shell access can still call `shepherd spawn` directly. Real gating
would mean Shepherd owning the pilot's whole tool surface, which is a larger
...
shepherd ws create NAME -C ROOT [-d DESCRIPTION] [--json]
shepherd ws rename ID NAME
shepherd ws reorder ID --up|--down
shepherd ws archive ID
shepherd ws root add ID PATH
shepherd ws root set ID OLD NEW
shepherd ws root rm ID PATH
...
### Slice 2 · the `manage-shepherd` skill

A second embedded skill beside `learn-shepherd`, following the same pattern:
`skills/manage-shepherd/SKILL.md` plus an `embed.go` exposing `Instructions`.

`learn-shepherd` teaches a human to press keys. `manage-shepherd` teaches an agent
to drive the CLI. It must establish:

- the noun model, and that a workstream organizes but does not coordinate;
- `shepherd list --json` as the only orientation step, run before every action;
...
- that workstream `notes.md` under `artifact_dir` is the durable place to record
  decisions;
- that Shepherd reports process truth only, so the pilot must never describe a
...
durable conversation identity needed to resume. Its turns are Shepherd's own
...
The pilot can shell out. It can call any `shepherd` verb regardless of what actor label
Shepherd attaches, so an authorizer rule would be theater: the agent is
indistinguishable from the human at the CLI boundary, and any environment marker
Shepherd injects can be unset by the process it marked.
...
`shepherd spawn` is the hard case. It is the pilot's most useful verb and its largest
consequence, so v1 does not teach it in the skill; it waits for the
Shepherd-rendered confirmation described above.
...
2. [x] Add `skills/manage-shepherd` and its embed, with a test asserting the
   instructions name the CLI-only mutation rule and the no-hand-editing rule.
   Installed into the Shepherd home as `AGENTS.md`, a `CLAUDE.md` pointer, and
   `skills/manage-shepherd/SKILL.md`, never overwriting an edited file.
...
4. [ ] Add `shepherd pilot` for a headless conversation from the CLI, including
...
6. [ ] Only then consider a spawn confirmation surface, which is what would let
   the pilot be taught `shepherd spawn` at all.
