...
// skills/manage-shepherd/SKILL.md tells the pilot which keys to read, and the
...
// documentedStates is the set skills/manage-shepherd promises. A state the pilot
...
// that enumerates the shepherd list --json keys. Reading the shipped text rather than
...
func TestSkillDocumentsOnlyFieldsTheJSONSurfaceEmits(t *testing.T) {
	...
	for _, field := range promised {
		if !known[field] {
			t.Errorf("SKILL.md promises the pilot a %q key that shepherd list --json does not emit", field)
		}
	}
	...
	for tag := range known {
		if !documented[tag] {
			t.Logf("shepherd list --json emits %q, which SKILL.md does not document", tag)
		}
	}
}
...
func TestSkillDocumentsEveryStateTheCLICanReport(t *testing.T) {
	for state := range documentedStates {
		if !strings.Contains(manageshepherd.Skill, "`"+state+"`") {
	...
	for state := range documentedStates {
		if !strings.Contains(manageshepherd.Agents, "`"+state+"`") {
