#!/bin/sh
# Check that the version Shepherd reports agrees with the tag people installed.
#
...
# without bumping: v0.4.0 then ships a binary whose `shepherd --version` says 0.3.6,
...
source_version="$(
	sed -n 's/^var version = "\([^"]*\)"$/\1/p' "$root/cmd/shepherd/main.go"
)"
if [ -z "$source_version" ]; then
	echo "check-version: could not read the version from cmd/shepherd/main.go" >&2
...
if ! printf '%s' "$source_version" | grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+(-[0-9A-Za-z.-]+)?$'; then
	echo "check-version: version $source_version in cmd/shepherd/main.go is not semver" >&2
	exit 1
fi
...
if ! grep -qF 'go install github.com/ez-gz/shepherd/cmd/shepherd@latest' "$root/README.md"; then
	echo "check-version: README no longer installs cmd/shepherd@latest" >&2
...
if [ "$tag_version" != "$source_version" ]; then
	echo "check-version: tag $tag does not match version $source_version in cmd/shepherd/main.go" >&2
